...
### Module Structure (PEP 8)

Order of elements within a Python file:

1. **Module docstring**
2. **`__future__` imports** (must precede all other imports)
3. **Module dunders** (`__version__`, `__author__`)
4. **Imports** (grouped with blank lines between groups):
   - Standard library
   - Third-party
   - Local/application
5. **`__all__`** (after imports, since it references imported symbols)
6. **Constants** (`ALL_CAPS`)
7. **Private helper functions** (`_underscore_prefix`)
8. **Classes and public functions** (two blank lines between top-level definitions)
...
# Acceptable — specific imports
from os.path import join

# Avoid — unclear dependencies, pollutes namespace
from os import *
```

| Check | Severity | Rationale |
| --- | --- | --- |
| `from X import *` in non-`__init__.py` | MEDIUM | Unclear dependencies, namespace pollution |
| Missing `__all__` in public API module | LOW | Consumers cannot distinguish public vs private names |
| `__future__` import after other imports | MEDIUM | Must be first import per PEP 263 |
| Module dunders after imports | LOW | PEP 8 ordering violation |
...
The src layout is recommended because it ensures tests run against the
*installed* version of the package, not the working directory files. This
catches packaging errors (missing files, broken imports) before release.

**Flat Layout** (acceptable for applications and scripts):
...
| Check | Severity | Rationale |
| --- | --- | --- |
| Distributable package uses flat layout | LOW | src layout catches packaging errors earlier |
| No `pyproject.toml` present | MEDIUM | PEP 621 is the standard for project metadata |

### Package Organization

| Check | Severity | Rationale |
| --- | --- | --- |
| `__init__.py` contains business logic | MEDIUM | Keep `__init__.py` minimal (empty is fine); use it only for public API re-exports |
| Underscores instead of subpackages | LOW | Prefer `library.plugin.foo` over `library.foo_plugin` for namespacing |
| Module exceeds one responsibility | MEDIUM | One module = one responsibility; split when a module serves unrelated concerns |
