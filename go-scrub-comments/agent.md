...
- **Iter 2 (skill gate):** First Phase 2 response MUST include a `Skill("comment-scrub-playbook")` call (per Hard Rule -1) alongside the first batch of Reads. **Per-Edit trim test (Hard Rule 0):** before each Edit, mentally strip the function-name restatement; if any "why"/edge-case/platform/spec/algorithm/cross-reference content remains, the Edit MUST be a trim (replace block with remaining content) — not a full delete. Multi-line blocks above unexported funcs almost always trim, not delete.
