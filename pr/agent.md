...
- **Input is the User Message.** When it is empty and tools are available, obtain
  it once with the branch diff via `git diff <base>...HEAD`; otherwise output exactly `NO INPUT`.
...
The entire response is the pr text in the OUTPUT FORMAT from system.md:
