...
- **Phase 1 (1 iter):** Parallel `Glob **/*.rs` + pattern search for LLM vocabulary/step labels. Prefer `rg --type rust -n` via Bash (faster, gitignore-aware); fall back to squad `Grep` if `rg` is absent. Filter out `target/`, `.git/`, `.claude/`, vendored dirs. Do NOT re-run discovery. No Bash `find`/`grep` — `rg` only.
