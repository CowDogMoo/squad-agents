---
name: go-tests
description: "Raises Go test coverage to a target (default 75% per package) by discovering uncovered packages, writing idiomatic table-driven _test.go files, and iterating until the target is met or budget is reached. Use when asked to add Go tests, raise coverage, fill test gaps, or test untested packages. Always analyzes coverage even if the target is already met."
...
pattern (Anthropic, "Building Effective Agents"). The orchestrator is the
skill `enqueue-coverage-targets-go`; the shared skill `test-writer-honesty`
supplies the non-negotiable discipline rules that keep you from destroying
existing tests or producing a dishonest final report. Load BOTH skills on
iteration 1 and follow their instructions exactly.
...
returns, with `${SQUAD_COVERAGE_TARGET:-75}` resolved to your coverage
target — **75** unless the caller specifies otherwise.
...
**Language bindings for `test-writer-honesty`:** test-file glob `*_test.go`;
new-test grep `\+func Test`; build `go build ./...`; test `go test ./...`;
coverage `go test -cover ./...`.

Per-package coverage target: 75% applies to ALL packages including `cmd/*`.
...
In one line: load both skills (iter 1), run the orchestrator's discovery
command (iter 2), then drain `/tmp/squad-targets.txt` in parallel Read/Write
batches of 3–5 packages until empty or budget reached. Final verify:
...
0. **Queue is the universe of work.** `/tmp/squad-targets.txt` is the
   COMPLETE set of valid target packages; packages NOT in the queue are
   at or above target — writing tests for them is FORBIDDEN waste. Empty
   queue → run final verify and emit the report. Do not browse the repo.
   (Run 1 burned iterations on `agent` 97.5%, `metrics` 93%, `tools`
   91.6% — none in queue — and skipped queued `routine/service` 72.2%.)
...
0b. **Per-package iteration cap: 8.** At 8 iterations on one package, STOP:
   list untested functions under Skipped Functions, MOVE TO THE NEXT QUEUE
   PACKAGE. Run 3 burned ~67 iter/pkg on 3 packages, never touched 17 others.
1. **Only create or modify `_test.go` files.** Never edit non-test source.
   Untestable without a refactor → Skipped Functions, "requires source refactor."
...
1e. **Editing Go files: respect file structure.** An `Edit` adding a test
   to an existing `_test.go` must NOT contain a fresh `package` line or
   `import (...)` block — add a new import by editing the existing block;
   append a new test as a top-level function after an existing one.
   Symptoms: "imports must appear before other declarations" (appended
   `import (...)`), "duplicate Test* declaration" (redeclared function).
1e-i. **NEVER insert code INSIDE an existing test function.** Add a NEW
   top-level `func Test*` after the previous one's `}`, OR a new entry to
   a `tests := []struct` slice. Re-inserting `t.Parallel()` panics
   ("called multiple times"); redeclaring `dir :=`/`ctx :=` fails ("no new
   variables on left side of :="). Prefer a new `func TestSomething_NewCase`;
   avoid `Edit` anchors inside another function's body.
1f. **Report fields are bound to `git diff` output, not internal state.**
   Binding rules (`test-writer-honesty` §3-§4): Files Touched = file list from
   `git diff --stat`; Tests Added (numeric) = per-package count of `+func Test*`
   lines from `git diff -U0 -- '*_test.go' | grep -E '^(\+func Test|diff --git)'`;
   Tests Written = only NEW `func Test*` names you added (not modifications);
   After % = real `go test -cover` re-measurement or "not measured". Quote actual
   `git diff --stat` output verbatim in the report. The numeric Tests Added column
   MUST equal the count of names in the Tests Written list for that package.
...
2a. **A failure in a file you did NOT touch is PRE-EXISTING/FLAKY — never
   edit it.** Empty queue ⇒ empty diff ⇒ EVERY failure is pre-existing.
   Re-run the one package alone: `go test -run <TestName> ./<pkg>/`.
   Passes alone ⇒ flaky timeout under parallel compile load (big packages
   like `tools` take ~60s); record under Validation as "pre-existing
   flaky failure, not reproduced in isolation" and STOP. Editing an
   untouched, passing test to silence a flake is a §10 contortion AND a
   rule-1 violation — the run that triggered this rule wrapped
   leak-detector `TestBashTool_NoSkillEnvWhenStackEmpty` in
   `SQUAD_SKILL_DIR= bash -c '...'`, forcing its asserted-empty value
   true so it could no longer catch the leak it exists to catch. NEVER
   make a test pass by neutering what it asserts.
...
7. **Report coverage delta — measured or "not measured", never fabricated.**
   The orchestrator wrote the baseline to `/tmp/squad-pkg-cov.out` — copy
   those numbers into the Before column verbatim. The After column MUST come
   from a real `go test -cover ./... 2>&1 | grep coverage:` re-measurement
   run as one of your LAST 3 iterations. The words "improved", "increased",
   "higher", "likely", "partial", "<92%", or any prose in place of a number
   are FORBIDDEN — write the actual percent or the literal string "not
   measured" (run 1 wrote "improved" for every row; that is the failure mode
   this rule names).
8. **Table-driven tests for 2+ cases.** Single-case tests don't need tables.
9. **Strict 1:1 test file naming — enforced before every Write.** The stem
   of `<X>_test.go` MUST exactly match an existing source file `<X>.go` in
   the same package; if `<X>.go` does not exist, `<X>_test.go` is INVALID.
   Violations the user rejected in prior runs: `run_extra_test.go` (no
   source `run_extra.go`); `service_state_test.go` (no `service_state.go`
   — tests of `State` belong in `service_test.go` because `State` is
   declared in `service.go`). FORBIDDEN suffix/stem patterns regardless of
   source presence: `_extra_test.go`, `_extras_test.go`,
   `_additional_test.go`, `_additions_test.go`, `_more_test.go`,
   `_coverage_test.go`, `_cov_test.go`, `_supplemental_test.go`,
   `_misc_test.go`, `_helpers_test.go` (unless `_helpers.go` exists), and
   any thematic "<topic>_test.go" where `<topic>.go` does not exist.
   Build-tagged source: `foo_darwin.go` → `foo_darwin_test.go` with the
   matching `//go:build` tag. **Escape hatch:** if `<X>_test.go` already
   exists and `Edit` cannot anchor your additions after 3 retries, SKIP
   the package and list each untested function under Skipped Functions
   with reason "existing test file Edit anchors exhausted." Do NOT create
   a sibling `<X>_extra_test.go` / `<X>_more_test.go` to dodge the failure
   — that sibling is itself a rule-9 violation and will be rejected.
   Per-package iteration cap is rule 14.
...
12. **Setup/mutate callbacks accept `t *testing.T` as parameter** — do not close over outer `t`.
13. **Goroutine-safe test helpers.** `sync/atomic` or `sync.Mutex` for mutable fields.
...
16. **Assert on error content, not just existence.** Check error message substrings.
17. **Per-package target:** 75% for every package, including `cmd/*`. No carve-outs.
...
19. **Load `Skill("enqueue-coverage-targets-go")` AND `Skill("test-writer-honesty")`**
    on iteration 1 (parallel). Do NOT load `Skill("score-coverage-and-report-gaps")`
    — its five-phase loop is what the orchestrator-workers pattern replaces.
...
20a. **Reserve the last 5 iterations for verify+report.** At iter
    ...
    then the report. Run 1 hit iter 200 mid-write — no verify, no real After numbers.
...
21. **Test name must match the branch the body exercises.** Naming a test
    `TestX_WhenBudgetExceeded` claims the body drives `X` into that branch.
    If the branch gates on `errors.Is(err, metrics.ErrBudgetExceeded)`, your
    `err` MUST BE that sentinel or wrap it with `%w` — a plain
    `fmt.Errorf("budget exceeded")` fails `errors.Is`, hits the early-return,
    and the name lies about which path ran. Re-read the predicate; if you
    can't satisfy it, rename or skip. (See `test-writer-honesty` §11.)
