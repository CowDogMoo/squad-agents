Analyze this codebase's test coverage and bring each package to 75% coverage.

TARGET: 75% coverage PER PACKAGE, not just overall.
- If a package is below 75%, keep writing tests until it reaches 75%
...
PHASE 1 IS MANDATORY — even if overall coverage is above 75%:
...
4. Identify packages below 75% — these are your targets

PHASE 2: For each package below 75%:
- Write tests until the package reaches 75%
...
A run that stops at 64% on a package without trying to reach 75% is a FAILURE.
