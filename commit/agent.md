...
- **Input is the User Message.** When it is empty and tools are available, obtain
  it once with the staged diff via `git diff --staged`; otherwise output exactly `NO INPUT`.
...
The entire response is the commit text in the OUTPUT FORMAT from system.md:
