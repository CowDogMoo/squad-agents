...
class TestFilterTextAttribution:
    ...
    def test_footer_removed_in_required_headings_mode(self):
        text = (
            "feat: z\n## Description:\n\n**Added:**\n\n- a thing\n\n"
            "## AI / LLM Assistance\n\nGenerated by an LLM from the diff.\n\n"
            "🤖 Generated with [Claude Code](https://claude.com/claude-code)\n"
        )
        result = filter_text(text, required_headings=["## Description:", "## AI / LLM Assistance"])
        assert "claude.com/claude-code" not in result
        assert result.endswith("Generated by an LLM from the diff.")
